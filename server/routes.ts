...
import { sendWelcomeEmail, sendPaymentConfirmationEmail, sendRenewalConfirmationEmail } from "./email-service.js";
...
export async function registerRoutes(app: Express): Promise<Server> {
    ...
    try {
      ...
      if (merchant) {
        ...
        if (merchant.accessEndDate) {
          ...
          // Check if merchant has a valid VIP plan that overrides accessEndDate
          const hasValidVipPlan = merchant.planStatus === "vip" && 
                                  merchant.planValidity && 
                                  new Date(merchant.planValidity) > now;
          
          if (accessEnd <= now && merchant.status === "active" && !hasValidVipPlan) {
            // Auto-suspend merchant with expired access only if they don't have a valid VIP plan
  ...
  app.get("/api/appointments/:id", authenticateToken, async (req, res) => {
    try {
      ...
      if (!appointment) {
        return res.status(404).json({ message: "Agenda de Belezamento não encontrado" });
      }
  ...
  app.delete("/api/appointments/:id", authenticateToken, async (req, res) => {
    try {
      ...
      res.json({ message: "Agenda de Belezamento removido com sucesso" });
    } catch (error) {
  ...
  app.post("/api/appointments/:id/cancel", authenticateToken, async (req, res) => {
    try {
      ...
      res.json({ message: "Agenda de Belezamento cancelado com sucesso", appointment });
    } catch (error) {
  ...
  app.post("/api/appointments/:id/reschedule", authenticateToken, async (req, res) => {
    try {
      ...
      res.json({ message: "Agenda de Belezamento reagendado com sucesso", appointment });
    } catch (error) {
  ...
  app.post("/api/appointments/:id/late", authenticateToken, async (req, res) => {
    try {
      ...
      res.json({ message: "Agenda de Belezamento marcado como atrasado", appointment });
    } catch (error) {
  ...
  app.post("/api/appointments/:id/no-show", authenticateToken, async (req, res) => {
    try {
      ...
      res.json({ message: "Agenda de Belezamento marcado como falta", appointment });
    } catch (error) {
  ...
  app.post("/api/client/appointments/:id/cancel", authenticateToken, async (req, res) => {
    try {
      ...
      const response = {
        message: "Agenda de Belezamento cancelado com sucesso",
        appointment,
        cancellationFee: {
          hasFee,
          amount: feeAmount,
  ...
  app.post("/api/pix/create-payment", authenticateToken, async (req, res) => {
    try {
      ...
      const paymentData = {
        ...
        description: `Agenda de Beleza Renovação Plano VIP`,
        statement_descriptor: "AGENDA DE BELEZA",
        payment_method_id: "pix",
        payer: {
          email: merchantEmail,
        },
        external_reference: `vip_renewal_${merchantEmail}_${Date.now()}`,
        notification_url: `${baseUrl}/api/pix/notification`,
        metadata: {
          merchant_email: merchantEmail,
          plan_type: "vip_renewal",
          business_type: "beauty_salon",
          service_description: "Renovação Agenda de Beleza"
        }
      };
  ...
  app.get("/api/pix/payment-status/:paymentId", authenticateToken, async (req, res) => {
    try {
      ...
      // A atualização da assinatura será feita exclusivamente pelo webhook /api/pix/notification
    ...
    try {
      ...
      const paymentData = {
        ...
        metadata: {
          ...
          plan_type: "vip",
          business_type: "beauty_salon",
          service_description: "Agenda de Beleza"
        }
      };
  ...
  app.post("/api/pix/notification", async (req, res) => {
    try {
            ...
            if (mpPayment.status === "approved" && (mpPayment.external_reference?.includes("vip_plan_") || mpPayment.external_reference?.includes("vip_renewal_"))) {
              ...
              if (merchant) {
                ...
                await storage.updateMerchant(merchant.id, {
                  planStatus: "vip",
                  planValidity: vipEndDate,
                  accessEndDate: vipEndDate,
                  paymentStatus: "paid",
                  status: "active",
                  lastPaymentDate: now,
                  nextPaymentDue: vipEndDate
                });

                console.log(`[PIX Webhook] Pagamento aprovado para o comerciante: ${merchantEmail}. Assinatura ${isRenewal ? 'renovada' : 'concedida'} por 30 dias.`);

                // Send confirmation email
                const amount = mpPayment.transaction_amount;
                const paymentDate = new Date(mpPayment.date_approved);
                const planName = "Plano VIP (30 dias)";
