...
export class SQLiteStorage implements IStorage {
  ...
  async getMerchantsAccessStatus(): Promise<{
    ...
    return {
      active: allMerchants.filter(m => 
        m.status === "active" && 
        (!m.accessEndDate || new Date(m.accessEndDate) > now)
      ).length,
      expired: allMerchants.filter(m => 
        m.accessEndDate && new Date(m.accessEndDate) <= now
      ).length,
      paymentPending: allMerchants.filter(m => m.status === "payment_pending").length,
      totalRevenue: allMerchants.filter(m => m.paymentStatus === "paid").reduce((total, m) => total + (m.monthlyFee || 0), 0),
    };
  }
