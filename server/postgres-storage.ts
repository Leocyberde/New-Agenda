...
export class PostgreSQLStorage implements IStorage {
  ...
  async processExpiredAccess(): Promise<number> {
    ...
    const expiredMerchants = await this.db.select().from(merchants)
      .where(and(
        ...
        sql`${merchants.accessEndDate} IS NOT NULL AND ${merchants.accessEndDate} <= ${nowString}`
      ))
      .execute();
    ...
    for (const merchant of expiredMerchants) {
      ...
      if (!hasValidVipPlan) {
        ...
        console.log(`Merchant ${merchant.id} (${merchant.email}) has valid VIP plan, skipping expiration.`);
      }
    }
  ...
  async suspendMerchantAccess(merchantId: string): Promise<Merchant | undefined> {
    ...
    // Get current merchant data to check VIP status
    const merchant = await this.getMerchant(merchantId);
    if (!merchant) return undefined;
    
    // Check if merchant has a valid VIP plan
    const now = new Date();
    const hasValidVipPlan = merchant.planStatus === "vip" && 
                            merchant.planValidity && 
                            new Date(merchant.planValidity) > now;
    
    // Don't suspend if merchant has valid VIP plan
    if (hasValidVipPlan) {
      console.log(`Merchant ${merchantId} has valid VIP plan, not suspending access.`);
      return merchant;
    }
